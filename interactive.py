...
if __name__ == "__main__":
    ...
    # Step 5: Demonstrate prediction on new, unseen data using user input
    ...
    if trained_model:
        while True:
            try:
                ...
                temp_input = input("Enter temperature in °C (e.g., 22.5): ")
                if temp_input.lower() == 'quit': break
                temperature = float(temp_input)

                humidity_input = input("Enter humidity in % (e.g., 55.0): ")
                if humidity_input.lower() == 'quit': break
                humidity = float(humidity_input)

                wind_speed_input = input("Enter wind speed in m/s (e.g., 3.1): ")
                if wind_speed_input.lower() == 'quit': break
                wind_speed = float(wind_speed_input)

                time_input = input("Enter time of day (0-23, e.g., 17): ")
                if time_input.lower() == 'quit': break
                time_of_day = int(time_input)

                day_input = input("Enter day of the week (0=Mon, 6=Sun, e.g., 6): ")
                if day_input.lower() == 'quit': break
                day_of_week = int(day_input)
                
                occupants_input = input("Enter number of occupants (1-5, e.g., 3): ")
                if occupants_input.lower() == 'quit': break
                num_occupants = int(occupants_input)

                hvac_input = input("Is HVAC on? (True/False): ")
                if hvac_input.lower() == 'quit': break
                is_hvac_on = hvac_input.lower() in ('true', 't', '1')

                appliance_input = input("Is a major appliance on? (True/False): ")
                if appliance_input.lower() == 'quit': break
                is_major_appliance_on = appliance_input.lower() in ('true', 't', '1')
